...
catch_entries_commun = function(K_stages, N_subsets, f, ratio_Delta_star_d1, ordering, increasing_theta, seed){
  ...
  if(K_stages<2){
    stop("The number of stages must be > 1")
  }
  ...
  if(as.integer(seed) != seed){
    ...
    print(paste("The mseed was transformed into integer: seed=", seed, sep=""))
  }
...
catch_entries_MT = function(K_stages, N_subsets, f, l, u, ratio_Delta_star_d1, type_outcome, param_outcome=NA,
                         ...
                         nb_required=NA, ordering, increasing_theta=FALSE, nsim=1000, seed=42){
  catch = catch_entries_commun(K_stages, N_subsets, f, ratio_Delta_star_d1, ordering, increasing_theta, seed)
  K_stages = catch[[1]]
  N_subsets = catch[[2]]
  f = catch[[3]]
  ...
  return(list(K_stages, N_subsets, f, l, u, ratio_Delta_star_d1, type_outcome, param_outcome, n_max, incl_rate, 
              cens_rate, param_cens, med_cur_c, HR, nb_required, ordering, increasing_theta, nsim, seed))
}
...
magnusson_turnbull = function(stage_cur, keep=NA, N_subsets, f, Y, I, l, u, ratio_Delta_star_d1, ordering, increasing_theta=FALSE){
  Z = Y/sqrt(I)
  if(stage_cur==0){
...
incl_and_update_patients = function(incl_rate, duration, follow, time_event, trt, biom_group, lost_censor, time_cens, keep, f, cens_rate, param_cens, med_cur_c, HR){
  ...
  follow = pmin(follow+new_incl, time_cens)
  duration = duration + new_incl
  ind_event = (time_event <= follow)
  time_min = pmin(time_event, follow, na.rm=TRUE)
  nb_events = sum(ind_event)
  return(list(follow,duration,trt,biom_group,time_event,ind_event,time_min,nb_events,lost_censor,time_cens))
}
...
sim_one_OS_MT = function(K_stages, N_subsets, f, l, u, ratio_Delta_star_d1, incl_rate, cens_rate, param_cens, 
                         med_cur_c, HR, nb_required, ordering, increasing_theta=FALSE){
  reject = NA
  nb_events = 0
  duration = rexp(1,incl_rate)
  follow = c()
  time_event = c()
  ind_event = c()
  trt = c()
  ...
  return(list("reject"=reject, "keep"=keep, "stage"=k, "duration"=duration, "nb_patients"=length(trt)))
}


#########
### Simulation of n design with Magnusson and Turnbull for survival data
sim_trials_OS_MT = function(K_stages, N_subsets, f, l, u, ratio_Delta_star_d1, incl_rate, cens_rate, param_cens, 
                            med_cur_c, HR, nb_required, ordering, increasing_theta=FALSE, nsim=1000, seed=42){
  set.seed(seed) 
  prob_rejec = 0
  prob_accep = 0
  list_keep = list()
...
sim_magnusson_turnbull = function(K_stages, N_subsets, f, l, u, ratio_Delta_star_d1, type_outcome, param_outcome=NA,
  ...
  if(type_outcome=="survival"){
    return(sim_trials_OS_MT(K_stages, N_subsets, f, l, u, ratio_Delta_star_d1, incl_rate, cens_rate, param_cens, 
                                med_cur_c, HR, nb_required, ordering, increasing_theta, nsim, seed))  
  }
